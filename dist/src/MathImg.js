var MathImg = /** @class */ (function () {
    ...
    //este el codigo tricolor en horizontal
    MathImg.toTricolor = function (img) {
        var arrImage = img.getArrayImg();
        var sal = this.initArray(img.getHeight(), img.getWidth());
        var sal = this.initArray(img.getWidth(), img.getHeight());
        var tercio = img.getHeight() / 3;
        for (var i = 0; i < tercio; i++) {
            for (var j = 0; j < img.getWidth(); j++) {
                sal[0][i][j] = arrImage[0][i][j];
                sal[1][i][j] = 0;
                (2 * tercio);
        ...
        for (var i = (2 * tercio); i < (tercio * 3); i++) {
            for (var j = 0; j < img.getWidth(); j++) {
                sal[0][i][j] = 0;
                sal[1][i][j] = 1.50 * arrImage[1][i][j];
                sal[2][i][j] = 0;
            }
        }
        return sal;
    ...
    MathImg.toDesfaceX = function (img, desx) {
        //variable que guarda el arreglo 3d de la imagen de color
        var arrImage = img.getArrayImg();
        //variable donde guardamos la salida
        var sal = this.initArray(img.getWidth(), img.getHeight());
        ...
        for (var i = 0; i < fila; i++) {
            for (var j = 0; j < cols; j++) {
                sal[0][i][j] = arrImage[0][i][j + (desx * Math.sin(2) * Math.cos(Math.PI / 2))];
                sal[1][i][j] = arrImage[1][i][j + desx];
                sal[2][i][j] = arrImage[2][i][j + desx];
            }
        }
        /*
          - desx * Math.sin(1/2)  - desx * Math.cos(1/2)
        ...
        for (let i = 0; i < fila; i++) {
              for (let j = 0; j < cols; j++) {
                sal[1][i][j] = arrImage[1][i][j];
                if ((j - desx) >= 0) {
                  sal[0][i][j] = arrImage[0][i - desx][j + desx];
                }
                else {
                  sal[0][i][j] = arrImage[0][i][j];
                }
